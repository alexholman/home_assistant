...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    async_add_devices([HeatpumpThermostat(
        hass, name, heater_entity_id, sensor_entity_id, min_temp, max_temp,
        target_temp, ac_mode, min_cycle_duration, cold_tolerance,
        hot_tolerance, keep_alive, initial_operation_mode)])


class HeatpumpThermostat(ClimateDevice):
    """
    Representation of a Heatpump Thermostat device.
	Desired behavior in heating mode is:
		1) When thermostat is set to below 61 device turns off
		2) When thermostat is set above 61 switches to HEAT mode and sets target temperature
		3) When room temp is hot_tolerance > set temp and in HEAT mode -> switches to FAN mode
		4) When room temp is cold_tolerance < set temp and in FAN mode -> switches to HEAT mode
    """

    def __init__(self, hass, name, heater_entity_id,
        ...
        async_track_state_change(
            hass, heater_entity_id, self._async_sensor_changed)
        async_track_state_change(
